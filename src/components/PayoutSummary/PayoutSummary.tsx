...
import { PayoutSummaryWrapper } from "styles/StyledComponents/PayoutSummaryWrapper";
import { PayoutOrderSummaryResponse } from "typescript/interface/report.interface";
import CommonDatePicker, { StyledCommonDatePicker } from "ui/Datepicker/CommonDatePicker";
import PayoutTableRow from "./PayoutTableRow";
import { useMutation } from "react-query";
import { payoutOrderSummary } from "api/functions/report.api";

const CustomSelect = lazy(() => import("ui/Filter/CustomSelect"));
...
type PayoutSummaryProps = {
  datePickerProps: React.ComponentProps<typeof CommonDatePicker>;
  selectedSortBy: "ASC" | "DESC";
  onChangeSortBy: (val: "ASC" | "DESC") => void;
  reports?: PayoutOrderSummaryResponse["docs"];
  isLoading: boolean;
  searchProps: React.ComponentProps<typeof InputFieldCommon>;
};
const PayoutSummary = ({
  selectedSortBy,
  onChangeSortBy,
        ...
        <Box className="payout_summary">
          ...
          <Box className="sort_filter">
            <CustomSelect
              ...
              onChange={(e) => {
                onChangeSortBy?.(
                  e.target.value as PayoutSummaryProps["selectedSortBy"]
                );
              }}
            >
              ...
              {sortByFilters.map((sortByFilter) => (
                <MenuItem
                  key={sortByFilter?.value}
                  value={sortByFilter?.value}
                  className="menu_item"
                  // selected={selectedSortBy === sortByFilter?.value}
                >
                  {sortByFilter?.name}
                </MenuItem>
              ))}
          ...
          >
            <TableHead className="mainTable_header">
              <TableRow>
                ...
                <TableCell>Orders</TableCell>
                <TableCell>Details</TableCell>
                <TableCell align="left" className="status_th">
                  Status
                </TableCell>
                <TableCell>Action</TableCell>
              </TableRow>
            ...
            <TableBody>
              {reports?.map((row) => (
                <PayoutTableRow key={row?.createdAt} report={row} />
              ))}
            </TableBody>
          </Table>
          {!isLoading && reports?.length === 0 ? (
            <NotFoundResult text="No reports found" />
          ) : null}
        </TableContainer>
      </HomeTableWrapper>
    </PayoutSummaryWrapper>
